...
type RandomEffect<'a> =
  | RandomEffect of (System.Random -> 'a)
...
module RandomEffect =
  ...
  let inline map f (RandomEffect random) = RandomEffect(fun r -> random r |> f)
  ...
  let tuple2 (RandomEffect r1) (RandomEffect r2) = RandomEffect(fun r -> (r1 r, r2 r))
  ...
  let inline sequence (rs: _ RandomEffect seq) =
    RandomEffect(fun r -> rs |> Seq.map (apply r))
